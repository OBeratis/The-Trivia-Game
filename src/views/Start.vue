...
const onSuccess = user => {
  let objJsonUser = JSON.parse(user)

  store.commit("setUsername", objJsonUser.user.username)
  ...
  <main class="container mx-auto px-4">
    ...
    <form>
      <div class="p-3">
        ...
        <button
          type="button"
          class="border bg-blue-500 text-white px3 rounded"
          @click="onGetUserClick"
        >
          Get User
        </button>
      </div>
      <div class="p-3">
        ...
        <select class="border" @change="onChangeDifficult($event)">
          <option
            v-for="item in dropdownDifficultItems"
      ...
      <div class="p-3">
        ...
        <select class="border" @change="onChangeCategory($event)">
          <option
            v-for="item in dropdownCategoryItems"
            v-bind:value="item.id"
            :key="item.id"
          >
            {{ item.text }}
      ...
      <div class="p-3">
        ...
        <select class="border" @change="onChangeType($event)">
          <option
            v-for="item in dropdownTypeItems"
            v-bind:value="item.id"
            :key="item.id"
          >
            {{ item.text }}
      ...
      <button
        class="border bg-blue-500 text-white px-3 rounded"
    ...
    <!-- <div v-if="displayError" class="bg-red-500 text-white">
      ...
      <p>
        {{ username }} - {{ numberOfQuestions }} - {{ selectDifficults }} -
        {{ selectCategories }}
      </p>
    </div> -->
  </main>
